...
const deselectedOptions = ['AS 로마', '유벤투스', '맨체스터 유나이티드', '레알 마드리드', '첼시', '토트넘'];
...
const Autocomplete = () => {
  ...
  return (
    ...
    <div className="autocomplete-wrapper">
      <InputContainer>
        <input type="text" value={inputValue} defaultValue={inputValue} onChange={handleInputChange} onKeyUp={handleInputChange}></input>
  ...
  return (
    <DropDownContainer>
      ...
      {options.map((option, idx) => {
        return (
          <li key={idx} onClick={() => handleComboBox(option)} className={selected === idx ? 'selected' : ''}>
            {option}
          </li>
        );
      })}
